...
export const deleteProfile = async (req, res) => {
    try {
        const userId = req.user._id;
        console.log("delete called for ", userId);
        const user = await User.findById(userId);
        const hash = await bcrypt.hash(user.email, 10);
        const safeHash = hash.replace(/\//g, "_");
        const tombstoneEmail = `${user.email}__deleted__${safeHash}`;
        ...
        // console.log("updated tombstone for ", userId);
        await Message.updateMany(
            { $or: [{ senderId: userId }, { receiverId: userId }] },
            { deletedAt: new Date() }
        );
        // console.log("updated messages for ", userId);
        res.cookie("jwt", "", { maxAge: 0 });
