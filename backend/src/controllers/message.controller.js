...
export const getMessages = async (req, res) => {
  try {
    ...
    // Check if the other user is soft-deleted
    const otherUser = await User.findOne({ _id: usertoChatid, deletedAt: null });
    if (!otherUser) {
    ...
    // Try Redis first (filter out soft-deleted messages)
    if (
      cached.length &&
      (!before || new Date(before) > new Date(oldestCachedMessage?.createdAt))
    ) {
      messages = cached
        .map(JSON.parse)
        .filter(
          (msg) =>
            (!before || new Date(msg.createdAt) < new Date(before)) &&
            (!msg.deletedAt) // skip soft-deleted messages
        )
        .slice(-Number(limit));
    } else {
      // Fetch from DB (exclude soft-deleted messages)
      const query = {
        deletedAt: null, // Do not include soft-deleted messages
        $or: [
